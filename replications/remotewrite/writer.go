	...
	for {
		...
		if res == nil {
			if errorIsTimeout(err) {
				w.logger.Debug(fmt.Sprintf("client timed out after %s", w.clientTimeout))
				res = &http.Response{}
			} else {
				// nil response that is not a timeout could be due to a malformed URL - cannot proceed
				return err
			}
		}
		...
		// statusCode may be 0 for client timeouts, but the error message will reflect that a timeout was reached.
		if err := w.configStore.UpdateResponseInfo(ctx, w.replicationID, res.StatusCode, errMsg); err != nil {
			return err
		}
