...
func createToTransformation(id execute.DatasetID, mode execute.AccumulationMode, spec plan.ProcedureSpec, a execute.Administration) (execute.Transformation, execute.Dataset, error) {
	...
	if !ok {
		return nil, nil, fmt.Errorf("dependencies for InfluxDB not set")
	}
...
func NewToTransformation(ctx context.Context, d execute.Dataset, cache execute.TableBuilderCache, toSpec *ToProcedureSpec, deps Dependencies) (x *ToTransformation, err error) {
...
func writeTable(ctx context.Context, t *ToTransformation, tbl flux.Table) (err error) {
	...
	return tbl.Do(func(er flux.ColReader) error {
		...
		for i := 0; i < er.Len(); i++ {
			...
			} else if fieldValues, err = t.fn.Eval(t.Ctx, t.deps, i, er); err != nil {
				return err
			}
